...
> {
  ...
  for (const observable of config.observables) {
    console.log("checking observable...", observable);
    if (
      observable.updateType == param.updateType &&
      observable.market == param.market
    ) {
      console.log("applicable observable found!");
      console.log(
        "proposing safe-tx to safe-multisig address:",
        config.adminSafeAddr
      );
      ...
      console.log("skip: observable not applicable");
    }
  }
  return undefined;
}
